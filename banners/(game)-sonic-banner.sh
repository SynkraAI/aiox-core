#!/bin/bash
# Sonic the Hedgehog — LUIZ FOSC version
# 10-frame animation: Sonic runs, collects rings, spin dash

GREEN='\033[0;32m'
BLUE='\033[0;34m'
WHITE='\033[1;37m'
NC='\033[0m'

luizfosc_logo() {
echo ""
echo -e "\033[38;5;27m ██╗     ██╗   ██╗██╗███████╗    ███████╗ ██████╗ ███████╗ ██████╗${NC}"
echo -e "\033[38;5;33m ██║     ██║   ██║██║╚══███╔╝    ██╔════╝██╔═══██╗██╔════╝██╔════╝${NC}"
echo -e "\033[38;5;39m ██║     ██║   ██║██║  ███╔╝     █████╗  ██║   ██║███████╗██║     ${NC}"
echo -e "\033[38;5;45m ██║     ██║   ██║██║ ███╔╝      ██╔══╝  ██║   ██║╚════██║██║     ${NC}"
echo -e "\033[38;5;51m ███████╗╚██████╔╝██║███████╗    ██║     ╚██████╔╝███████║╚██████╗${NC}"
echo -e "\033[38;5;87m ╚══════╝ ╚═════╝ ╚═╝╚══════╝    ╚═╝      ╚═════╝ ╚══════╝ ╚═════╝${NC}"
echo ""
echo -e "\033[38;5;33m ⚡ Gotta go fast! ⚡${NC}  ${WHITE}${SYS_NAME:-AIOS Core} ${GREEN}v${SYS_VERSION:-2.1}${NC}"
echo -e "\033[38;5;27m ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
}

# Frame 1 - Sonic standing (far left)
frame1() {
cat << 'EOF'

                                    🟡    🟡    🟡
       ⬛⬛🟦🟦🟦
      ⬛🟦🟦🟦🟦🟦⬛
      ⬛🟦🟦🟦🟦🟦🟦⬛
       ⬛⬛🏽🏽🏽⬛
      ⬛🏽⬜⬛🏽⬜⬛⬛
      ⬛🏽🏽🏽🏽🏽⬛
       ⬛🟦🟦🟦🟦⬛
      ⬛🟥🟦🟦🟦🟥⬛

EOF
}

# Frame 2 - Sonic running pose 1
frame2() {
cat << 'EOF'

                                 🟡    🟡    🟡
          ⬛⬛🟦🟦🟦
         ⬛🟦🟦🟦🟦🟦⬛
         ⬛🟦🟦🟦🟦🟦🟦⬛
          ⬛⬛🏽🏽🏽⬛
         ⬛🏽⬜⬛🏽⬜⬛⬛
         ⬛🏽🏽🏽🏽🏽⬛
        ⬛🟦🟦🟦🟦🟦⬛
         ⬛🟥⬛    ⬛🟥⬛

EOF
}

# Frame 3 - Sonic running pose 2 (legs switched)
frame3() {
cat << 'EOF'

                              🟡    🟡    🟡
              ⬛⬛🟦🟦🟦
             ⬛🟦🟦🟦🟦🟦⬛
             ⬛🟦🟦🟦🟦🟦🟦⬛
              ⬛⬛🏽🏽🏽⬛
             ⬛🏽⬜⬛🏽⬜⬛⬛
             ⬛🏽🏽🏽🏽🏽⬛
              ⬛🟦🟦🟦🟦⬛
           ⬛🟥⬛      ⬛🟥⬛

EOF
}

# Frame 4 - Sonic grabs ring!
frame4() {
cat << 'EOF'

                           🟡    🟡
                 ⬛⬛🟦🟦🟦
                ⬛🟦🟦🟦🟦🟦⬛
                ⬛🟦🟦🟦🟦🟦🟦⬛
                 ⬛⬛🏽🏽🏽⬛
                ⬛🏽⬜⬛🏽⬜⬛⬛
                ⬛🏽🏽😏🏽🏽⬛
                 ⬛🟦🟦🟦🟦⬛  ✨🟡✨
                  ⬛🟥⬛⬛🟥⬛

EOF
}

# Frame 5 - Sonic running faster (speed lines)
frame5() {
cat << 'EOF'

                        🟡
    ─ ─ ─            ⬛⬛🟦🟦🟦
    ─ ─ ─           ⬛🟦🟦🟦🟦🟦⬛
    ─ ─ ─           ⬛🟦🟦🟦🟦🟦🟦⬛
    ─ ─ ─            ⬛⬛🏽🏽🏽⬛
    ─ ─ ─           ⬛🏽⬜⬛🏽⬜⬛⬛
    ─ ─ ─           ⬛🏽🏽🏽🏽🏽⬛
                      ⬛🟦🟦🟦🟦⬛
                    ⬛🟥⬛      ⬛🟥⬛

EOF
}

# Frame 6 - Sonic crouches for spin dash
frame6() {
cat << 'EOF'


    ─ ─ ─ ─ ─
    ─ ─ ─ ─ ─
    ─ ─ ─ ─ ─         ⬛🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─        ⬛🟦🟦🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─        ⬛🏽🏽🏽🏽🏽🏽⬛
    ─ ─ ─ ─ ─         ⬛🟦🟦🟦🟦🟦⬛
                        ⬛🟥⬛⬛🟥⬛


EOF
}

# Frame 7 - Spin dash charge
frame7() {
cat << 'EOF'


    ─ ─ ─ ─ ─ ─
    ─ ─ ─ ─ ─ ─
    ─ ─ ─ ─ ─ ─          ⬛⬛⬛⬛⬛
    ─ ─ ─ ─ ─ ─        ⬛🟦🟦🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─ ─        ⬛🟦🟦🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─ ─          ⬛⬛⬛⬛⬛
                        💨💨💨


EOF
}

# Frame 8 - Spin ball flying!
frame8() {
cat << 'EOF'



    ─ ─ ─ ─ ─ ─ ─ ─
    ─ ─ ─ ─ ─ ─ ─ ─              ⬛⬛⬛⬛
    ─ ─ ─ ─ ─ ─ ─ ─            ⬛🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─ ─ ─ ─            ⬛🟦🟦🟦🟦⬛
    ─ ─ ─ ─ ─ ─ ─ ─              ⬛⬛⬛⬛
                        💨💨💨💨💨

EOF
}

# Frame 9 - Sonic lands in victory pose
frame9() {
cat << 'EOF'

                                          ⬛⬛🟦🟦🟦
                                         ⬛🟦🟦🟦🟦🟦⬛
                                         ⬛🟦🟦🟦🟦🟦🟦⬛
                                          ⬛⬛🏽🏽🏽⬛
                                         ⬛🏽⬜⬛🏽⬜⬛⬛
                                         ⬛🏽🏽😏🏽🏽⬛
                                       ⬛🏽⬛🟦🟦🟦⬛🏽⬛
                                          ⬛🟥⬛⬛🟥⬛

EOF
}

# Frame 10 - Victory + rings count
frame10() {
cat << 'EOF'

                                      ✨              ✨
                                          ⬛⬛🟦🟦🟦
                                    ✨   ⬛🟦🟦🟦🟦🟦⬛   ✨
                                         ⬛🟦🟦🟦🟦🟦🟦⬛
                                          ⬛⬛🏽🏽🏽⬛
                                    ✨   ⬛🏽⬜⬛🏽⬜⬛⬛
                                         ⬛🏽🏽😏🏽🏽⬛  ✨
                                       ⬛🏽⬛🟦🟦🟦⬛🏽⬛
                                          ⬛🟥⬛⬛🟥⬛
                                      🟡 × 50        ✨

EOF
}

# Animation
printf "\033[?25l"

printf "\033[H\033[2J"; frame1; sleep 0.25
printf "\033[H\033[2J"; frame2; sleep 0.15
printf "\033[H\033[2J"; frame3; sleep 0.15
printf "\033[H\033[2J"; frame4; sleep 0.3
printf "\033[H\033[2J"; frame5; sleep 0.15
printf "\033[H\033[2J"; frame6; sleep 0.25
printf "\033[H\033[2J"; frame7; sleep 0.2
printf "\033[H\033[2J"; frame8; sleep 0.15
printf "\033[H\033[2J"; frame9; sleep 0.35
printf "\033[H\033[2J"; frame10; sleep 0.35

# Final
printf "\033[H\033[2J"
luizfosc_logo
printf "\033[?25h"
sleep 0.8
