# Projetos Ativos

| # | Projeto | Status | Agente/Squad | Última Sessão | INDEX |
|---|---------|--------|-------------|---------------|-------|
| 1 | **ensinio** | **v5.0 — Sheets auditado, 75/77 GHL sync OK** | **ensinio-whatsapp-prospector** | **2026-03-12** | [INDEX](ensinio/INDEX.md) |
| 2 | gui-avila-mind | Transcrição limpa disponível | squad-creator-pro | 2026-03-07 | [INDEX](gui-avila-mind/INDEX.md) |
| 3 | luiz-fosc-mentoria | Research completa | tech-search | 2026-03-06 | [INDEX](luiz-fosc-mentoria/INDEX.md) |
| 4 | luiz-fosc-mind-clone | Research INPI concluída | tech-search | 2026-03-05 | [INDEX](luiz-fosc-mind-clone/INDEX.md) |
| 5 | storytelling-masters-fosc | Research Fala Magnética | tech-search | 2026-03-05 | [INDEX](storytelling-masters-fosc/INDEX.md) |
| 6 | high-ticket-sales | 25 livros mapeados | tech-search | 2026-03-04 | [INDEX](high-ticket-sales/INDEX.md) |
| 7 | **renner-silva** | **Clone v1.2 pronto (9.5/10)** | **Kaizen + squad-chief** | **2026-03-09** | [INDEX](renner-silva/INDEX.md) |
| 8 | **designcode-ui** | **32/32 componentes + 29/29 Storybook stories (100%)** | **design-chief** | **2026-03-11** | [INDEX](designcode-ui/INDEX.md) |
| 9 | **meta-ads-prospector** | 🔄 **5 stories revisadas e aprovadas — pronto para @dev** | **@pm/@architect/@po/@sm → @dev** | **2026-03-12** | [INDEX](~/CODE/Projects/meta-ads-prospector/.aios/INDEX.md) |
| 10 | **🏆 EPIC: Squad Ecosystem Quality** | **🟡 Ready for Commitment — 7 stories, 17-24.5h, 3 P0 + 4 P1** | **@squad-chief → @dev** | **2026-03-12** | [INDEX](.aios-core/development/epics/2026-03-squad-ecosystem-quality/) |
| 11 | **naval-ravikant-mind** | **Mind Clone Elite 95% — falta smoke test** | **mind-cloning** | **2026-03-11** | [INDEX](naval-ravikant-mind/INDEX.md) |
| 12 | **tabgroup-vault** | 🔄 **Extensão Brave/Chrome — backup de tab groups** | **—** | **2026-03-12** | [INDEX](~/CODE/Projects/tabgroup-vault/.aios/INDEX.md) |
| 13 | **garimpoai** | 🔄 **Assistente IA para licitações públicas brasileiras (PNCP)** | **—** | **2026-03-12** | [INDEX](~/CODE/Projects/garimpoai/.aios/INDEX.md) |

## Backlog (Futuros Projetos)

| # | Projeto | Origem | Status | Próximo Passo |
|---|---------|--------|--------|---------------|
| B1 | mkt-movemente | cmux workspace | Backlog | Definir escopo e objetivos |
| B2 | ai-traffic | cmux workspace | Backlog | Definir escopo e objetivos |
| B3 | app-zona-genialidade | cmux workspace | Backlog | Definir escopo e objetivos |
| B4 | app-artes-ensinio | cmux workspace | Em andamento (42% contexto, $8.21) | Continuar sessão Lovable |
| B5 | ~~app-extrair-dados-whatsapp-ensinio~~ | cmux workspace | Cancelado — deletado | — |
| B6 | casa-fundamental | cmux workspace | Backlog | Definir escopo e objetivos |
