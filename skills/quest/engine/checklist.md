...
### Condition state (optional field for conditioned items)

Items with a `condition` field in the pack gain an additional `condition_state` field in the quest-log to track whether the condition has been evaluated:

| `condition_state` | Meaning | Item `status` after |
|-------------------|---------|---------------------|
...
- When a condition is evaluated (§6), the `condition_state` is updated based on the user's answer.
- `guide.md` §2 uses `condition_state` to distinguish actionable pending items from unresolved ones: only items with `condition_state == applicable` (or items without a `condition` field) are candidates for next-mission selection.
- Items without a `condition` field in the pack do NOT have a `condition_state` field — absence means unconditionally applicable.

**Fallback for missing or malformed `condition` fields:**
- **Missing `condition` field:** The item is treated as **unconditional** — it starts as `pending` with no `condition_state` field and is immediately eligible for next-mission selection (guide.md §2) and progress tracking. This is the default for most items.
- **Malformed `condition` field** (e.g., empty string `""`, non-string type, unrecognized condition format): Treat the item as **unconditional/pending** — same as if the `condition` field were absent. Do NOT error or crash. Log a warning: `"Warning: item {id} has malformed condition '{value}', treating as unconditional."` This ensures forward compatibility when new condition types are added. This rule is consistent with guide.md §2 (which only selects items with `condition_state == applicable` or items without a `condition` field).
...
3. Build the `items` map: iterate ALL phases in the pack, for each item add an entry keyed by `item.id` with `{ status: pending }`. For items with a `condition` field, also set `condition_state: unresolved`. Then evaluate conditions: for each item with a `condition` field, invoke the condition evaluation rules from §6. Condition decisions update both `condition_state` and `status` — apply them to the items map before calculating stats. This ensures that items which don't apply to this project are excluded from `items_total` from the start. **Deduplication note:** When SKILL.md Step 7 runs "create quest-log + initial scan", the scan's §6 invocation will see the `condition_state` already set by Create and skip re-asking (§6 step 1). This makes Create the single owner of first-run condition prompts.
...
5. **Always recalculate stats** via xp-system. Never trust saved `stats` values. Pass the current pack and the quest-log items to xp-system, write the returned stats to `quest_log.stats`. This runs AFTER promotion (step 4) so promoted items are counted.
   **Contract — progress bar visual consistency:** The stats calculated here (`percent`, `items_done`, `items_total`, `items_skipped`) feed the progress bar rendered by ceremony.md §2 (loading sequence), ceremony.md §7 (Resumption Banner), guide.md §5 (Quest Log View per-phase bars), and guide.md §6 (summary view). All four visual locations share one contract: `█` for filled, `░` for empty, bar width 20 chars, `round()` rounding. Any change to bar rendering or stats calculation MUST be propagated to all locations in the same commit.
...
**Edge case — no conditioned items:** If no items in the pack have a `condition` field, or all conditioned items already have `condition_state` set to `applicable` or `not_applicable` (permanently resolved), this section performs no work. The §5 (scan) and §2 (create) callers detect this when the conditioned-items list is empty before iteration begins. The for-each loops exit cleanly with no mutations — no explicit guard is needed. This is documented here to confirm: condition evaluation never errors on an empty input, and it never re-evaluates permanently resolved items (step 1 skips them).
