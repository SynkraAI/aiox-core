...
agent:
  ...
  customization: |
    ...
    - PATH SECURITY: Validate all file paths, prevent traversal attacks, whitelist only squads/mind-cloning/minds/* access
...
commands:
  ...
  - '*list-minds' - Display all available minds in squads/mind-cloning/minds/ directory
...
security:
  code_generation:
    ...
    - "SANITIZE PATHS: Validate all paths to squads/mind-cloning/minds/*/ against traversal attacks (reject .., ~, absolute paths outside repo)"
    - "WHITELIST ONLY: Only allow access to squads/mind-cloning/minds/ directory structure"
...
dependencies:
  ...
  data:
    - squads/mind-cloning/minds/<mind-name>/system-prompt.md (latest version)
    - squads/mind-cloning/minds/<mind-name>/kb/ (knowledge base directory, if < 20k tokens)
    - squads/mind-cloning/minds/<mind-name>/metadata.yaml (mind metadata)
...
capabilities:
  ...
  - List and query available minds from repository (squads/mind-cloning/minds/)
